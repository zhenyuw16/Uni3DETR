...
def bboxes_nms_merge_only(classes, scores, bboxes, scores_threshold=0.25,
    nms_threshold=0.45, overlapped_fn=overlapped_boxes_3d_fast_poly, appr_factor=10.0,
    attributes=None):
    ...
    for i in range(scores.size-1):
        if keep_bboxes[i]:
            ...
            # boxes_mean = np.mean(overlaped_bboxes, axis=0)
            bboxes[i][:] = boxes_mean[:]
    ...
    if attributes is not None:
        attributes = attributes[idxes]
    return classes, scores, bboxes, idxes, #attributes

def nms_boxes_3d_merge_only(class_labels, detection_boxes_3d, detection_scores,
    overlapped_thres=0.5, overlapped_fn=overlapped_boxes_3d_fast_poly, appr_factor=10.0,
